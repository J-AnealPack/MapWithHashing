...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private void createNewRep(int hashTableSize) {
        assert hashTableSize > 0 : "Violation of: hashTableSize must be greater than 0";
        ...
        // Populate the hash table with instances of Map2 for each bucket.
        for (int i = 0; i < hashTableSize; i++) {
            this.hashTable[i] = new Map2<K, V>();
        }

        // Reset the total size of the map to 0 as it's newly initialized.
        this.size = 0;
    }
    ...
    public final void add(K key, V value) {
        ...
        // Retrieve the bucket (map) where the key should be located
    ...
    public final Pair<K, V> remove(K key) {
        ...
        // Remove the key-value pair from the bucket and store the removed pair
        Pair<K, V> removedPair = bucket.remove(key);

        // Decrement the overall size of the hashTable
        this.size--;
    ...
    public final Pair<K, V> removeAny() {
        ...
        for (int i = 0; i < this.hashTable.length && removedPair == null; i++) {
            Map<K, V> currBucket = this.hashTable[i];
            if (currBucket.size() != 0) {
                removedPair = currBucket.removeAny();
            }
        }
        this.size--;
        return removedPair;
    ...
    public final boolean hasKey(K key) {
        ...
        // Remove the key-value pair from the bucket and store the removed pair
        boolean keyFound = bucket.hasKey(key);
